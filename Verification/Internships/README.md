...
Runs the real Razor pages on localhost with a local fixture API. Checks Student-only access, internship visibility without a saved career path, empty/error states, filtering/search, Employee separation, screening links and employer category form rendering/binding. Also checks Employee/Internship badges and searchable category values on the employer vacancies list. No hosted API is contacted.
